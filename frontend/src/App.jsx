...
const plans = [
  { name: "Gratuit", key:"free", price: "0€", limit: 1 },
  { name: "Starter", key:"starter", price: "4,99€/mois", limit: 10 },
  { name: "Pro", key:"pro", price: "9,99€/mois", limit: 50 },
  { name: "Label", key:"label", price: "49€/mois", limit: 9999 },
];
...
function App() {
  ...
  return <div className="app">
    ...
    <main>
      <header className="topbar"><input placeholder="Rechercher..."/><div className="account"><User size={18}/> {user?.publicName || user?.public_name || "Rayan Studio"}</div></header>
...
function Landing({setPage}){return <section className="page"><div className="hero"><div><span className="pill">Startup V10</span><h1>Upload réel branché côté frontend.</h1><p>Création d’artiste, upload audio en FormData, lecture audio, API tracks, Stripe checkout et admin préparés.</p><button className="primary" onClick={()=>setPage("artistCreate")}>Créer artiste</button><button onClick={()=>setPage("upload")}>Publier un son</button></div><div className="phone"><div>React → API</div><div>FormData Upload</div><div>Audio Player</div><div className="phonePlayer">MVP connectable</div></div></div></section>}
...
function UploadPage({uploadTrack, artists}){return <section className="page"><h1>Publier une musique</h1><div className="notice">Tu dois être connecté et avoir créé un artiste. L’audio est envoyé au backend puis vers stockage local/R2 selon configuration.</div><form className="panel" onSubmit={uploadTrack}><label>Artiste<select name="artistId" required>{artists.map(a=><option key={a.id} value={a.id}>{a.public_name}</option>)}</select></label><label>Titre<input name="title" required placeholder="Titre du morceau"/></label><label>Genre<select name="genre"><option>Rap</option><option>Afro</option><option>R&B</option><option>Électro</option></select></label><label>Description<textarea name="description"/></label><label>Fichier audio<input name="audio" type="file" accept="audio/*" required/></label><div className="notice">En envoyant, tu confirmes que tu possèdes les droits et qu’il n’y a aucun sample non autorisé.</div><button className="primary">Uploader</button></form></section>}

function Creator({tracks,artists}){return <section className="page"><h1>Dashboard créateur</h1><div className="stats"><Info title={tracks.length} text="Titres affichés"/><Info title={artists.length} text="Artistes"/><Info title="Stripe" text="Checkout prêt"/><Info title="R2/S3" text="Stockage prêt"/></div><div className="list">{tracks.map(t=><Row key={t.id} t={t} setNow={()=>{}}/>)}</div></section>}
...
function Production(){return <section className="page"><h1>Production</h1><div className="grid"><Info title="Frontend" text="Vercel"/><Info title="Backend" text="Render/Railway"/><Info title="DB" text="Supabase PostgreSQL"/><Info title="Audio" text="Cloudflare R2/S3"/><Info title="Paiement" text="Stripe"/><Info title="Légal" text="CGU/RGPD"/></div></section>}

function Card({t,setNow}){return <div className="card"><div className="cover">{t.icon || "🎧"}</div><h3>{t.title}</h3><p>{t.artist_name || t.artist} · {t.genre}</p><button onClick={()=>setNow(t)}>▶</button></div>}
function Row({t,setNow}){return <div className="row"><div className="thumb">{t.icon || "🎧"}</div><div><b>{t.title}</b><p>{t.artist_name || t.artist} · {t.genre}</p></div><button onClick={()=>setNow(t)}>▶</button></div>}
function Info({title,text}){return <div className="info"><b>{title}</b><p>{text}</p></div>}

function Player({now}) {
  const src = now?.audio_url || "";
  return <div className="player">
    <div>{now ? `${now.icon || "🎧"} ${now.title} — ${now.artist_name || now.artist}` : "Aucun titre sélectionné"}</div>
